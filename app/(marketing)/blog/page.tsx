...
export default function BlogPage() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 font-sans antialiased text-secondary">
      ...
      <section className="bg-white border-b border-zinc-100 pt-20 pb-16 relative overflow-hidden italic">
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
            <div className="flex items-center justify-center gap-2 mb-6">
                ...
                <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-primary">Industry Insights</span>
                <div className="h-[1px] w-6 bg-primary" />
            </div>
      ...
      {/* Newsletter / CTA */}
      <section className="py-16 px-6 bg-secondary text-white relative overflow-hidden italic">
        <div className="max-w-xl mx-auto text-center relative z-10">
            <h2 className="text-2xl font-bold font-display uppercase italic mb-6">Stay ahead of the market.</h2>
            <p className="text-white/50 text-sm uppercase font-medium mb-12">
                Join 5,000+ local Maryland buyers receiving weekly auction alerts.
            </p>
            <div className="flex flex-col sm:flex-row gap-3 max-w-sm mx-auto">
                <input 
                    type="email" 
                    placeholder="EMAIL@DOMAIN.COM" 
                    className="flex-1 bg-white/5 border border-white/10 rounded-xl px-5 py-4 text-xs font-bold uppercase tracking-widest outline-none focus:bg-white/10 focus:border-primary transition-all"
                />
                <button className="bg-primary text-white px-8 py-4 rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-white hover:text-secondary transition-all shadow-xl shadow-primary/20">
                    Subscribe
                </button>
            </div>
        </div>
        <div className="absolute top-[-50px] left-[-50px] h-48 w-48 bg-primary/10 blur-[80px] rounded-full" />
      </section>
    </div>
  );
}

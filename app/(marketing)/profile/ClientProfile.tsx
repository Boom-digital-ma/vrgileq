    ...
    <div className="bg-zinc-50 min-h-screen font-sans antialiased text-secondary italic">
      ...
      {isAdminViewing && (
        <div className="bg-primary text-white py-2 px-6 text-center text-[10px] font-black uppercase tracking-[0.3em] flex items-center justify-center gap-3">
          <ShieldCheck size={14} /> Administrative Viewing Mode: Accessing {data.profile?.full_name || 'User'} dossiers
        </div>
      )}
      {/* SaaS Premium Header */}
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <aside className="lg:w-64 shrink-0">
            <nav className="flex flex-col gap-1.5 sticky top-32">
              ...
              ].filter(t => isAdminViewing ? !['payment', 'security'].includes(t.id) : true).map((tab) => (
                <button
                  key={tab.id}
                          ...
                          <div className="bg-white p-10 rounded-[40px] border border-zinc-100 shadow-sm space-y-10">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                  ...
                                  <div className="space-y-2">
                                      ...
                                      <input disabled={isAdminViewing} name="phone" type="tel" defaultValue={data.profile?.phone} className="w-full bg-zinc-50 border-2 border-zinc-100 rounded-2xl py-4 px-6 text-sm font-bold text-secondary focus:outline-none focus:border-primary/20 focus:bg-white transition-all italic outline-none disabled:opacity-50" />
                                  </div>
                              </div>
                              <div className="space-y-2">
                              ...
                              <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
                                  ...
                                  <div className="space-y-2">
                                      ...
                                      <input disabled={isAdminViewing} name="state" type="text" defaultValue={data.profile?.state} className="w-full bg-zinc-50 border-2 border-zinc-100 rounded-2xl py-4 px-6 text-sm font-bold text-secondary focus:outline-none focus:border-primary/20 focus:bg-white transition-all italic outline-none disabled:opacity-50" />
                                  </div>
                                  <div className="space-y-2">
                                      <label className="text-[10px] font-bold uppercase tracking-widest text-zinc-400 ml-4">ZIP</label>
                                      <input disabled={isAdminViewing} name="zip" type="text" defaultValue={data.profile?.zip_code} className="w-full bg-zinc-50 border-2 border-zinc-100 rounded-2xl py-4 px-6 text-sm font-bold text-secondary focus:outline-none focus:border-primary/20 focus:bg-white transition-all italic outline-none disabled:opacity-50" />
                                  </div>
                              </div>
                          </div>
                          {!isAdminViewing && (

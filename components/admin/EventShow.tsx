...
import { useShow, useList, useNavigation, useDelete, useForm, useInvalidate, useTable } from "@refinedev/core"
import { ArrowLeft, Plus, Loader2, Package, Gavel, Trash2, Edit, Save, Eye, Search } from "lucide-react"
...
export const EventShow = () => {
  ...
  // 2. Table Data - Controlled for nested view
  const resultTable = useTable({
    ...
    // CRITICAL: Disable URL sync to avoid conflict with event ID in URL
    syncWithLocation: false,
    meta: { select: "*, auction_images(*), bids(id)" }
  })

  // Refine v5 patterns - Extract tableQuery data
  ...
  // Extract pagination and filter controls with proper type casting
  // In Refine v5, pagination uses currentPage/setCurrentPage instead of current/setCurrent
  const {
    current = 1,
    setCurrent,
    currentPage = 1,
  ...
  // Use the available pagination method (v5 uses setCurrentPage)
  ...
  const editForm = useForm({ 
    ...
    queryOptions: { 
        ...
        meta: { select: "*, auction_images(*)" } 
    }
  })
  const editData = (editForm as any).queryResult?.data?.data;
    ...
    <div className="space-y-8 font-sans text-zinc-900 pb-20">
      ...
      <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Edit Lot Details">
        <form onSubmit={(e) => handleUpsert(e, selectedLotId!)} className="p-8 space-y-6">
            ...
            <div className="space-y-4">
                <div className="grid grid-cols-4 gap-4">
                    <div className="col-span-1"><label className={labelClasses}>Lot #</label><input name="lot_number" type="number" defaultValue={editData?.lot_number} key={editData?.lot_number} className={inputClasses} /></div>
                    <div className="col-span-3"><label className={labelClasses}>Product Title</label><input name="title" defaultValue={editData?.title} key={editData?.title} required className={inputClasses} /></div>
                </div>
                <div>
                    <label className={labelClasses}>Category</label>
                    <select name="category_id" defaultValue={editData?.category_id} key={`${selectedLotId}-${editData?.category_id}`} className={inputClasses}>
                        <option value="">Select a category...</option>
                        {categoriesList.map((cat: any) => <option key={cat.id} value={cat.id}>{cat.name}</option>)}
                    </select>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><label className={labelClasses}>Start Price ($)</label><input name="start_price" type="number" defaultValue={editData?.start_price} key={editData?.start_price} className={inputClasses} /></div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div><label className={labelClasses}>Min. Increment ($)</label><input name="min_increment" type="number" defaultValue={editData?.min_increment} key={editData?.min_increment} className={inputClasses} /></div>

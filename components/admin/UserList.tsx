...
export const UserList = () => {
  ...
  const handleEditSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = await adminUpdateUser(selectedId, data)
    if (res.success) {
  ...
  return (
    <div className="space-y-6 text-zinc-900 font-sans pb-20">
              ...
              <tr key={profile.id} className="hover:bg-zinc-50/50 transition-colors group text-zinc-900">
                ...
                <td className="px-8 py-6 text-center">
                  <span className={cn(
                      ...
                      profile.role === 'logistics' ? "bg-teal-50 text-teal-700 border-teal-100" :
                      profile.role === 'moderator' ? "bg-blue-50 text-blue-700 border-blue-100" :
                      "bg-zinc-50 text-zinc-500 border-zinc-200"
                  )}>{profile.role}</span>
                </td>
                <td className="px-8 py-6 text-right font-sans">
                  {profile.role === 'admin' || profile.role === 'moderator' || profile.role === 'logistics' ? (
                ...
                <td className="px-8 py-6 text-right font-sans">
                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all font-sans">
                        <button onClick={() => show("profiles", profile.id)} className="p-2 text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100 rounded-xl transition-all" title="View Detail"><Eye size={18} /></button>
                        <button onClick={() => handleEditClick(profile)} className="p-2 text-zinc-400 hover:text-teal-600 hover:bg-teal-50 rounded-xl transition-all" title="Edit Role"><Edit size={18} /></button>
                        <button onClick={() => { setSelectedId(profile.id); setIsDeleteOpen(true); }} className="p-2 text-zinc-300 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all" title="Delete"><Trash2 size={18} /></button>
                    </div>
                </td>
              </tr>
            ))}
      ...
      <Modal isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} title="Provision New User" maxWidth="max-w-md">
        <form onSubmit={handleCreateSubmit} className="p-8 space-y-6 font-sans">
            ...
            <div className="flex justify-end pt-4 font-sans">
                ...
                >
                    {loading ? <Loader2 size={18} className="animate-spin" /> : <><Save size={18} /> Provision Account</>}
                </button>
            </div>
        </form>
      </Modal>
      ...
      {/* EDIT MODAL */}
      <Modal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title="Modify User Access" maxWidth="max-w-md">
        {selectedProfile && (
            <form onSubmit={handleEditSubmit} className="p-8 space-y-6 font-sans">
                <div className="space-y-4 font-sans text-zinc-900">
                    <div>
                        <label className={labelClasses}>Account Identity</label>
                        <div className="relative">
                            <User className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-zinc-400" />
                            <input name="full_name" defaultValue={selectedProfile.full_name} required className={cn(inputClasses, "pl-10")} />
                        </div>
                    </div>

                    <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100 flex items-center gap-3">
                        <Mail className="text-zinc-300" size={16} />
                        <div>
                            <p className="text-[8px] font-black text-zinc-400 uppercase tracking-widest mb-0.5">Primary Email</p>
                            <p className="text-xs font-bold text-zinc-600 italic">{selectedProfile.email}</p>
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4 font-sans">
                        <div>
                            <label className={labelClasses}>Privilege Role</label>
                            <select name="role" defaultValue={selectedProfile.role} required className={inputClasses}>
                                <option value="client">Client</option>
                                <option value="logistics">Logistics</option>
                                <option value="moderator">Moderator</option>
                                <option value="admin">Administrator</option>
                ...
                <div className="flex justify-end pt-4 font-sans">
                    <button 
                        disabled={loading} 
                        className="bg-secondary text-white px-8 py-3 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-zinc-800 transition-all active:scale-95 shadow-lg"
                    >
                        {loading ? <Loader2 size={18} className="animate-spin" /> : <><Shield size={18} /> Update Privileges</>}
                    </button>
                </div>
            </form>
        )}

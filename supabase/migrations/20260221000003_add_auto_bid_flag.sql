-- 1. Add is_auto_bid column to bids table
ALTER TABLE public.bids 
ADD COLUMN IF NOT EXISTS is_auto_bid BOOLEAN DEFAULT false;

-- 2. Update place_bid_secure to INSERT new rows for auto-bids instead of updating in place
...
BEGIN
    ...
    -- RULE 1: Same bidder cannot bid twice in a row
    ...
    IF v_current_max_amount IS NOT NULL AND v_current_max_amount >= p_amount THEN
        ...
        -- A. Insert the Challenger's bid (Outbid immediately)
        ...
        -- B. Archive the Defender's previous active bid
        UPDATE bids
        SET status = 'outbid'
        WHERE auction_id = p_auction_id AND status = 'active' AND user_id = v_current_winner_id
        RETURNING stripe_payment_intent_id INTO v_old_pi_id;

        -- C. Insert the Defender's NEW auto-bid
        -- This ensures a new INSERT event is fired for Realtime to pick up
        INSERT INTO bids (
            auction_id, 
            user_id, 
            amount, 
